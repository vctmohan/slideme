Rails.application.routes.draw do
  ...
  get ':username' => 'users#show', as: :profile , :constraints => { :username => /[0-9A-Za-z\-\.]+/ }
  ...
  match ':username/new', :controller => 'editor', :action => 'new', :via => [:get], as: :new_deck, :constraints => { :username => /[0-9A-Za-z\-\.]+/ }
  match ':user/:slug/edit', :controller => 'editor', :action => 'edit', :via => [:get], :constraints => { :user => /[0-9A-Za-z\-\.]+/ }
  match ':user/:slug/live', :controller => 'live', :action => 'live', :via => [:get], as: :live, :constraints => { :user => /[0-9A-Za-z\-\.]+/ }
  match ':user/:slug', :controller => 'deck', :action => 'show', :via => [:get], as: :show_deck, :constraints => { :user => /[0-9A-Za-z\-\.]+/ }
